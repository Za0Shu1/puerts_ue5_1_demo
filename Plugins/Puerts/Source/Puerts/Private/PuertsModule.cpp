...
public:
    ...
    FString IPuertsModule::GetScriptRoot()
    {
        return  ScriptRoot;
    }
    ...
    virtual void MakeSharedJsEnv() override
    {
        ...
        if (NumberOfJsEnv > 1)
        {
            // 使用可配置的ScriptRoot而不是固定的"JavaScript"
            if (Settings.DebugEnable)
            {
                JsEnvGroup = MakeShared<puerts::FJsEnvGroup>(NumberOfJsEnv,
                    std::make_shared<puerts::DefaultJSModuleLoader>(Settings.ScriptRoot), std::make_shared<puerts::FDefaultLogger>(),
                    DebuggerPortFromCommandLine < 0 ? Settings.DebugPort : DebuggerPortFromCommandLine);
            }
            else
            {
                JsEnvGroup = MakeShared<puerts::FJsEnvGroup>(NumberOfJsEnv,Settings.ScriptRoot);
            }
        ...
        else
        {
            ...
            if (Settings.DebugEnable)
            {
                JsEnv = MakeShared<puerts::FJsEnv>(std::make_shared<puerts::DefaultJSModuleLoader>(Settings.ScriptRoot),
                    std::make_shared<puerts::FDefaultLogger>(),
                    DebuggerPortFromCommandLine < 0 ? Settings.DebugPort : DebuggerPortFromCommandLine);
            }
            else
            {
                JsEnv = MakeShared<puerts::FJsEnv>(Settings.ScriptRoot);
            }
...
void FPuertsModule::RegisterSettings()
{
    ...
    if (GConfig->DoesSectionExist(SectionName, PuertsConfigIniPath))
    {
        ...
        // 读取配置到ScriptRoot中
        GConfig->GetString(SectionName, TEXT("ScriptRoot"), Settings.ScriptRoot, PuertsConfigIniPath);
    }
